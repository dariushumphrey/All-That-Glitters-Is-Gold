...
public class StunningLucentScript : MonoBehaviour
{
    public int lucentGift; //Lucent cluster worth
    public int hitTriggerGoal = 10;
    public int currentHitCount = 0;
    public GameObject shotEffect; //VFX that plays on condition
    ...
    void Update()
    {
        if(currentHitCount >= hitTriggerGoal)
        {
            ShatterImmediate();
        }
    }

    void ShatterImmediate()
    {
